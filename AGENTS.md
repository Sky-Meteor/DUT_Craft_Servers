...
静态页面：展示 DUT_Craft Minecraft 服务器状态。无框架，纯 TypeScript + Vite + 手写 DOM 操作。界面为纯视觉方向（无世界观叙事）：编辑级排版——Fraunces 衬线大字与数字、暖纸（light）/ 暖夜（dark）双色、发丝线、柔和景深卡片网格。UI 文案与代码注释全部使用中文，新代码保持一致。
...
- `src/ui.ts` — 卡片（`.card`）渲染、地址 chip 点击复制、toast。
- `src/theme.ts` — 深色/浅色切换（`data-theme` 属性 + localStorage key `dutcraft-theme`，与 index.html 的防 FOUC 内联脚本共用）。
- `src/types.ts` — 共享类型。
...
- **不引入任何外部字体/CDN 资源**：展示字体 Fraunces（衬线可变，Roman+Italic latin 子集）自托管于 `assets/fonts/`，中文回退系统字体栈——为了大陆可达性与消除渲染阻塞。新增字体同样走自托管。
